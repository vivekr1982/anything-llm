...
2. After successful login, Run the following command to create a deployment using the Deployment Manager CLI:
  ...
  gcloud deployment-manager deployments create cloudwise-gpt-deployment --config gcp/deployment/gcp_deploy_anything_llm_with_env.yaml
...
```
gcloud compute instances get-serial-port-output cloudwise-gpt-instance
```

ssh into the instance

```
gcloud compute ssh cloudwise-gpt-instance
```

Delete the deployment
```
gcloud deployment-manager deployments delete cloudwise-gpt-deployment
```
